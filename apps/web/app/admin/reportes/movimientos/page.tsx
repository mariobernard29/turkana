  ...
  return (
    <div>
      ...
      <form className="mb-8 flex flex-wrap items-end gap-3">
        ...
        {(f.tipo || f.desde || f.hasta) && <Link href="/admin/reportes/movimientos" className="px-2 py-2 text-sm text-muted hover:text-ink">Limpiar</Link>}
      </form>
          ...
          <tbody>
            ...
            {rows.map((m) => (
              <tr key={m.id} className="border-b border-ink/5 last:border-0">
                ...
                <td className="px-6 py-3 text-muted">{m.method ? METHOD[m.method] ?? m.method : "—"}</td>
